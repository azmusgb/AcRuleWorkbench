...
| Viewer UX | Desktop workbench with structure/global definitions/inspector | UI shape is good; Functions now has a first-class global workspace and global-definition search filters actual rows. Deeper panels still depend on richer canonical data. | Add first-class object tree, rule packet, UDF packet, table packet, field packet, and runtime-impact panels. | P1 |
