...
| Selected rule inspector | Viewer inspector sections, rule detail API, `/api/v1/rules/{nodeId}/editor-model`, viewer RuleConfiguration packet panel | Selected-rule packets and the viewer now show canonical function schema/status evidence, source handles, reject mappings, ambiguity diagnostics, and field/page context through page-design packets. Full function-specific parameter schemas remain partial. | Complete function schemas and add deeper raw evidence drill-through. | P0 |
