...
| Rule List model | Structural nodes/edges, flat action names, selected-rule `editorModel`, `/api/v1/rule-lists`, viewer RuleConfiguration packet panel | Snapshot-wide Rule List and Rule Configuration objects expose parent Status Result, outgoing Action Lists, function schema, reject message/code mapping, raw source handles, and duplicate/ambiguous flat-inventory diagnostics. Full authoring-grade function-specific parameter schemas remain partial. | Add complete function-specific parameter schemas and deeper raw evidence drill-through for every rule configuration. | P0 |
| Selected rule inspector | Viewer inspector sections, rule detail API, `/api/v1/rules/{nodeId}/editor-model`, viewer RuleConfiguration packet panel | Selected-rule packets and the viewer now show canonical function schema/status evidence, source handles, reject mappings, and ambiguity diagnostics when available. Field/page context and full function-specific parameter schemas remain partial. | Expand field/page context, complete function schemas, and add deeper raw evidence drill-through. | P0 |
...
| UDF definitions | `/api/v1/fwd/udfs*`, `/api/v1/fwd/udfs/canonical`, viewer UDF evidence panel | Canonical UDF definitions now expose resource identity, resource evidence, field-list parameters, status-result evidence, private body candidate nodes, caller bindings, and caller-slot-to-field-list bindings with confidence. True authoring-grade private body reconstruction is still incomplete when native private-tree content is opaque. | Deepen Function resource interface/private rule-body parsing and promote candidate private nodes into full internal Rule List trees when payload evidence allows. | P1 |
...
| API contract | API v1 route catalog and OpenAPI | API is useful but still compatibility-oriented in places; canonical Editor concepts are not complete route contracts. | Add stable canonical endpoints while preserving compatibility aliases. OpenAPI examples must show Editor vocabulary and evidence classes. | P1 |
...
| Tests | API, export, sync, cache tests | Need golden tests for canonical Editor projections and function/UDF/table semantics. | Add fixture-backed snapshot tests and contract tests for canonical objects. | P0 |
...
- Function catalog hits set relationship kind, target type, parameter role, confidence, and runtime impact.
...
| Function catalog tests | Exact parameter role classification beats heuristics. |
...
| P2 | Add page/variant design context. | Page variant/FormID/dropout/OMR/field geometry is navigable from object tree. |
