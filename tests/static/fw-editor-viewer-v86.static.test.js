...
assert(viewerHtml.includes('data-ui-build="v86-fw-editor-viewer"'), 'viewer HTML should advertise v86 build marker');
assert(viewerJs.includes("const viewerStateBuild='v86-fw-editor-viewer'"), 'viewer JS state key should use v86 build marker');
...
assert(packageJson.includes('fw-editor-viewer-v86.static.test.js'), 'package test script should reference the v86 static test');
...
assert(coreTemplate.includes('data-ui-build="v86-fw-editor-viewer"'), 'core HTML template should advertise the current FW Editor Viewer shell');
...
assert(viewerJs.includes("truthyQueryFlag('apiHydrate')"), 'API hydration should be opt-in through apiHydrate for performance-sensitive browsing');
assert(viewerJs.includes('ruleUsageByFunctionName'), 'v86 should pre-index rules by function name instead of rescanning all rules per function/UDF');
assert(viewerJs.includes('model.ruleUsageByFunctionName?.get(lower(functionName))'), 'function usage lookup should use the precomputed function index');
assert(viewerJs.includes('model.ruleUsageByFunctionName?.get(lower(fnName))'), 'UDF caller lookup should use the precomputed function index');
assert(viewerJs.includes('nodesByScope') && viewerJs.includes('ruleNodesByScope') && viewerJs.includes('edgesByScope'), 'scope-local render paths should use indexed maps instead of repeated full-array filters');
assert(viewerJs.includes('model.visibleRowsCache'), 'Rule List visible rows should be cached per scope/query/filter/expansion state');
assert(viewerJs.includes('model.treeMatchCache'), 'tree search matching should be cached instead of recomputing recursive matches for each row');
assert(viewerJs.includes('globalNavigationCountsCache') && viewerJs.includes('productCountsCache'), 'global navigation/product counts should be cached after first computation');
assert(!viewerJs.includes('JSON.stringify(x),x.title'), 'search matching should not stringify full objects for every candidate row');

console.log('FW Editor Viewer v86 static checks passed.');
