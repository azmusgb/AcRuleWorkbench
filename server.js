...
const ALLOW_GENERATED_SIDECARS = args.has('--allow-generated-sidecars') || process.env.ALLOW_GENERATED_SIDECARS === '1';
...
function normalizeRequestPath(req) {
  ...
  const isViewerSidecar = /^ac-rule-viewer\..*\.json$/i.test(fileName);
  if (BLOCKED_FILE_NAMES.has(fileName) && !(ALLOW_GENERATED_SIDECARS && isViewerSidecar)) {
...
server.listen(PORT, HOST, () => {
  ...
  console.log('Static server is localhost-only by default and does not start the FWD API.');
  console.log('For live FWD data, run: powershell.exe -NoProfile -ExecutionPolicy Bypass -File .\\scripts\\start-fw-editor-viewer.ps1 -FwdPath C:\\path\\to\\fwd.cfd');
  if (ALLOW_GENERATED_SIDECARS) {
    console.log('Generated viewer sidecar JSON serving is ENABLED for this local session.');
  } else {
    console.log('Generated/private sidecar JSON is blocked. Use --allow-generated-sidecars only for local standalone exports.');
  }
});
