...
function compactGlobalDefinitionRowHtml(kind,row,options={}){
  ...
  return `<button class="global-view-row child definition-tree-row ${active?'active':''}" type="button" data-def-kind="${esc(kind)}" data-def-key="${esc(key)}" aria-current="${active?'true':'false'}" title="${esc(title)}"><span class="global-view-name"><b>${esc(name)}</b>${meta?`<small>${esc(meta)}</small>`:''}</span>${countHtml}</button>`;
}
...
function renderGlobalInventoryTreeBlock(){
  if(!model?.fwd)return '';
  const functionRows=buildGlobalFunctionDefinitions().slice(0,16);
  const selectionRows=buildSelectionListPacketDefinitions().slice(0,16);
  const tableRows=buildGlobalTableDefinitions().slice(0,16);
  const udfRows=buildUdfDefinitions().slice(0,16).map(row=>({key:row.key,name:row.displayName||row.name||row.key,type:row.type||'UDF',metric:list(row.callerRules).length}));
  const driverRows=buildGlobalDriverDefinitions().slice(0,12);
  const packetRows=buildRuleListPacketDefinitions().slice(0,10);
  const objectRows=buildObjectGraphDefinitions().slice(0,10);
  const runtimeRows=buildRuntimeImpactDefinitions().slice(0,10);
  const body=[
    compactResourceBucketTreeHtml(),
    compactGlobalDefinitionSectionHtml('udfs','UDFs',udfRows,{limit:12}),
    compactGlobalDefinitionSectionHtml('tables','Tables / SelectionLists',tableRows,{limit:12}),
    compactGlobalDefinitionSectionHtml('selection-lists','SelectionLists',selectionRows,{limit:10}),
    compactGlobalDefinitionSectionHtml('functions','Functions',functionRows,{limit:12}),
