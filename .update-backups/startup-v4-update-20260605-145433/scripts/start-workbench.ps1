...
while ((Get-Date) -lt $deadline) {
    ...
    if ($liveSeen -and (Test-EndpointOk -Url $ReadyHealthUrl)) {
...
'@
    ...
    try {
        ...
        Write-WbOk "Viewer open helper started. It will open the viewer after ready health responds, or after timeout if the API is live."
    }
    catch {
...
if ($SkipViewerRefresh) {
    if (-not (Test-Path -LiteralPath $viewerOutputPath -PathType Leaf)) {
        throw "-SkipViewerRefresh was specified, but the expected viewer file is missing: $viewerOutputPath. Rerun without -SkipViewerRefresh to generate it."
    }

    Write-WbOk "Skipping viewer refresh. Existing viewer file found: $viewerOutputPath"
}
else {
    Write-WbInfo "Generating static viewer before API startup: $viewerOutputPath"
    Write-WbInfo "Large FWD/CFD files can make this step take a while. Use -SkipViewerRefresh only when this file already exists."
...
# In foreground mode the server process blocks this script, so a small helper waits for
# ready health and then opens /viewer directly. If readiness times out but live health
# responded, it opens the viewer anyway so the UI can show the current loading state.
...
if (-not $NoBrowser -and -not $Detached -and -not $NoOpenWhenReady) {
    Start-WbViewerOpenHelper -LiveHealthUrl $liveHealthUrl -ReadyHealthUrl $readyHealthUrl -ViewerUrl $viewerUrl -TimeoutSeconds $ReadyTimeoutSeconds
}
