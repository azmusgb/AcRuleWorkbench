...
const storeKey='ac-rule-workbench-v62-17-global-inventory-counts';
...
function udfFilterBarHtml(){
  return `<div class="udf-filter-strip" role="group" aria-label="UDF filters">${['all','with-callers','defined','unparsed','relationship-only'].map(f=>`<button class="${state.udfFilter===f?'active':''}" type="button" data-udf-filter="${f}">${esc(udfFilterLabel(f))}</button>`).join('')}</div>`;
}
