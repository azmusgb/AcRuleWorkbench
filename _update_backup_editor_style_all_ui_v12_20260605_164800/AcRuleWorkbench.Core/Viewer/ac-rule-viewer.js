...
const storeKey='ac-rule-workbench-v62-18-udf-experience';
...
function renderUdfMasterDetail(){
  ...
  if(!rows.length){
    const hero=workspaceHeroHtml({
      ...
      caption:'No UDFs match the active search/filter. Clear search or choose All to restore the full inventory.',
      metrics:udfOverviewMetrics(allRows,[],null)
    });
    const body=`<div class="udf-explorer-shell empty-state">${udfFilterBarHtml(allRows)}${emptyHtml('No matching UDFs','Choose a broader UDF filter, clear search, or verify /api/v1/fwd/udfs returns rows.')}</div>`;
    $('content').innerHTML=workspacePageHtml('udfs',hero,workspaceSectionHtml('UDF inventory',body,{caption:'Filters apply to the confirmed global UDF inventory.'}),{split:true});
    return;
  }
  ...
  const hero=workspaceHeroHtml({
    eyebrow:'Global Inventory',
    title:'User Defined Functions',
    caption:'Browse all extracted UDF definitions, caller rules, parameter bindings, status results, internal rule body evidence, and parse confidence.',
    metrics:udfOverviewMetrics(allRows,rows,selected),
    chips:[
      {label:`Filter: ${udfFilterLabel(state.udfFilter)}`,tone:'blue'},
      q?{label:`Search: ${q}`,tone:'amber'}:null,
      {label:`Selected: ${udfShortName(selected)}`,tone:'green'}
    ].filter(Boolean),
    actions:`<button class="btn" type="button" data-action="open-global-detail" data-global-kind="udfs">Open full details</button>`
  });
  const detail=`<div class="udf-detail-stack polished">${udfInterfaceHtml(selected,callers)}${udfCallerRulesHtml(callers,selected)}${udfInternalRulesHtml(selected)}${udfEvidenceHtml(selected)}</div>`;
  const body=`<div class="global-explorer global-explorer-udfs udf-explorer polished v10"><aside class="global-index-panel workspace-card udf-index-panel">${udfFilterBarHtml(allRows)}${udfIndexHtml(rows,selected.key)}</aside><section class="global-detail-panel workspace-card workspace-card-primary udf-detail-panel">${detail}<div class="global-detail-actions"><button class="btn" type="button" data-action="open-global-detail" data-global-kind="udfs">Full details</button></div></section></div>`;
  $('content').innerHTML=workspacePageHtml('udfs',hero,body,{split:true});
}
