...
.DESCRIPTION
  ...
  - WorkingTree: use against C:\dev\AcRuleWorkbench. Local runtime assets, generated sidecars,
    docs, native DLLs, bin/, and obj/ are reported as package-excluded inventory, but the script
    exits successfully because those files can legitimately exist during local development.

  - SourcePackage: use against a source-clean staging folder or extracted source-clean zip. Package-
    excluded inventory is treated as a validation failure.

  Compatible with Windows PowerShell 5.1. Does not use [System.IO.Path]::GetRelativePath.
#>
[CmdletBinding()]
param(
...
function Test-IsUnderPath {
    ...
    return $normalized -eq $directory -or
        $normalized.StartsWith($directory + '\', [System.StringComparison]::OrdinalIgnoreCase) -or
        $normalized.IndexOf('\' + $directory + '\', [System.StringComparison]::OrdinalIgnoreCase) -ge 0
}
...
Get-ChildItem -LiteralPath $rootPath -Recurse -Force -File | ForEach-Object {
    ...
    foreach ($directory in $repoDirectories) {
        if (Test-IsUnderPath -RelativePath $relativePath -DirectoryPath $directory) {
            $findings.Add((New-Finding -Type 'RepoMetadata' -Rule $directory -Path $relativePath -Category 'Repository metadata')) | Out-Null
            return
        }
    }
    ...
    foreach ($directory in $buildDirectories) {
        if (Test-IsUnderPath -RelativePath $relativePath -DirectoryPath $directory) {
            $findings.Add((New-Finding -Type 'BuildOutput' -Rule $directory -Path $relativePath -Category 'Build output')) | Out-Null
            return
        }
    }
    ...
    foreach ($directory in $runtimeDirectories) {
        if (Test-IsUnderPath -RelativePath $relativePath -DirectoryPath $directory) {
            $findings.Add((New-Finding -Type 'RuntimeAsset' -Rule $directory -Path $relativePath -Category 'Native/runtime asset')) | Out-Null
            return
        }
    }
    ...
    foreach ($directory in $privateDirectories) {
        if (Test-IsUnderPath -RelativePath $relativePath -DirectoryPath $directory) {
            $findings.Add((New-Finding -Type 'PrivateAttachment' -Rule $directory -Path $relativePath -Category 'Private/local attachment')) | Out-Null
            return
        }
    }
    ...
    foreach ($directory in $dependencyDirectories) {
        if (Test-IsUnderPath -RelativePath $relativePath -DirectoryPath $directory) {
            $findings.Add((New-Finding -Type 'DependencyCache' -Rule $directory -Path $relativePath -Category 'Dependency cache')) | Out-Null
            return
        }
    }

    if ($generatedFileNames -contains $fileName) {
        $findings.Add((New-Finding -Type 'GeneratedEvidence' -Rule $fileName -Path $relativePath -Category 'Generated evidence')) | Out-Null
        return
    }

    if ($privateFileNames -contains $fileName) {
        $findings.Add((New-Finding -Type 'PrivateFwd' -Rule $fileName -Path $relativePath -Category 'Private FWD/config')) | Out-Null
        return
    }

    if ($buildExtensions -contains $extension) {
        $findings.Add((New-Finding -Type 'BuildExtension' -Rule $extension -Path $relativePath -Category 'Build/binary extension')) | Out-Null
        return
    }

    if ($localExtensions -contains $extension) {
        $findings.Add((New-Finding -Type 'LocalArtifact' -Rule $extension -Path $relativePath -Category 'Local artifact')) | Out-Null
        return
    }

    if ($archiveExtensions -contains $extension) {
        $findings.Add((New-Finding -Type 'ArchiveArtifact' -Rule $extension -Path $relativePath -Category 'Archive/package artifact')) | Out-Null
        return
    }

    if ($relativeUnix -match '^docs/.*\.(pdf|extracted\.txt)$') {
        $findings.Add((New-Finding -Type 'BlockedDocsReference' -Rule 'docs/.*\.(pdf|extracted\.txt)' -Path $relativePath -Category 'Confidential/source reference')) | Out-Null
        return
    }

    if ($relativeUnix -match '(^|/)ac-rule-viewer-live\.(html|css|js)$') {
        $findings.Add((New-Finding -Type 'GeneratedLiveViewer' -Rule 'ac-rule-viewer-live.(html|css|js)' -Path $relativePath -Category 'Generated viewer')) | Out-Null
        return
    }

    if ($relativeUnix -match '(^|/).*\.local\.json$') {
        $findings.Add((New-Finding -Type 'LocalConfig' -Rule '*.local.json' -Path $relativePath -Category 'Local config')) | Out-Null
        return
    }
}
