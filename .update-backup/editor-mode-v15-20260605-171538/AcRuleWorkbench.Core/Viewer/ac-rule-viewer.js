...
function fweditorDefinitionTreeHtml(kind,rows,selectedKey){
  ...
  const body=ordered.map(([group,items])=>{
    ...
    const itemHtml=items.slice(0,600).map(row=>{
      ...
      return `<button class="fweditor-tree-item ${selected?'selected':''}" type="button" data-def-kind="${esc(kind)}" data-def-key="${esc(row.key)}" title="${esc(row.name)}"><span class="fweditor-tree-glyph">${kind==='functions'?'ƒ':kind==='tables'?'▦':kind==='rule-lists'?'R':kind==='object-graph'?'◇':'◆'}</span><span class="fweditor-tree-label"><b>${esc(row.name)}</b><small>${esc(text(row.source||origin.origin||''))}${usage?` · ${fmt(usage)} usage`:''}</small></span>${warn?'<span class="fweditor-tree-warn">!</span>':''}</button>`;
    }).join('');
...
function fweditorGlobalMessageWindowHtml(kind,selected,rows,originUi){
  ...
  return `<section class="fweditor-message-window" aria-label="Message Window"><div class="fweditor-message-title">Message Window</div><table class="fweditor-message-table"><thead><tr><th>Sev</th><th>Object</th><th>Message</th></tr></thead><tbody>${messages.map(r=>`<tr><td><span class="sev ${lower(r.sev)}">${esc(r.sev)}</span></td><td>${esc(r.object)}</td><td>${esc(r.message)}</td></tr>`).join('')}</tbody></table></section>`;
}
...
function renderTop(){
  ...
  document.body.classList.toggle('fweditor-global-mode',isGlobalDefinitionView());
  if(!model||bootState.phase==='loading'){
    $('sourceSubtitle').textContent='Loading FWD snapshot...';
