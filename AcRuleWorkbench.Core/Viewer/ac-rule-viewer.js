...
function renderScopeInspector(s){const hotspots=scopedRuleNodes().filter(isHotspotNode).length;$('inspectorBody').innerHTML=`<details class="inspector-section" open><summary>Scope summary <span class="section-count">${fmt(s.structural)} rules</span></summary><div class="inspector-section-body"><div class="kv">${kv('Scope ID',esc(s.scopeId))}${kv('Kind',esc(s.kind))}${kv('Structural rules',fmt(s.structural))}${kv('Complex rules',fmt(hotspots))}${kv('Diagnostics',s.diags?`<span class="badge amber">${fmt(s.diags)}</span>`:'<span class="badge green">None</span>')}</div><div class="branch-actions mt-10"><button class="btn" type="button" data-action="largest-scope">Largest scope</button></div></div></details><details class="inspector-section" open><summary>How to work this scope</summary><div class="inspector-section-body"><ul class="evidence-list"><li>Select a rule row to inspect route and child branches.</li><li>Expand a rule to see outgoing action branches.</li><li>Use Audit when you need diagnostics, provenance, or extraction details.</li></ul></div></details>`;}
...
function activeSliceHtml(){
  const scope=currentScope();
  const isDocOrPage=scope&&/^(document|page)$/i.test(text(scope.kind));
  const view=({structure:'Structure','field-resolution':'Field Resolution',resources:'Resources',tables:'Tables',drivers:'Drivers',udfs:'UDFs',audit:'Audit'})[state.workspaceView]||state.workspaceView.replace(/-/g,' ').replace(/\b\w/g,c=>c.toUpperCase());
...
function wireOnboardingChecklist(){
  ...
  // Attach click handlers to tabs to support ARIA activation and focus transfer
  document.querySelectorAll('[role="tab"]').forEach(tab=>{
    tab.addEventListener('click', (ev)=>{
      const t = ev.currentTarget;
      const view = t.dataset.consoleTab;
      if(view){
        state.consoleView = view;
        renderShellStatePanels();
        saveState();
        // move focus to the panel body for screen reader users
        const panelId = t.getAttribute('aria-controls');
        const panel = panelId ? optionalElement(panelId) : null;
        const focusTarget = panel ? panel.querySelector('.console-body') || panel : null;
