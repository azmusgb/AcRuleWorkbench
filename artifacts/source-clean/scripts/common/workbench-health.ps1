...
function Wait-WbHttpEndpoint {
    ...
    Write-WbProgress "Waiting for ${Label}: $Url"
    ...
    while ((Get-Date) -lt $deadline) {
        ...
        if ($last.Ok) {
            ...
            Write-WbOk "$Label is responding after ${elapsedSeconds}s: $Url"
            return $true
        }

        if (($attempt % 5) -eq 0) {
            $elapsedSeconds = [Math]::Round(((Get-Date) - $started).TotalSeconds, 1)
            Write-WbProgress "Still waiting for $Label (${elapsedSeconds}s elapsed, timeout ${TimeoutSeconds}s)."
        }
