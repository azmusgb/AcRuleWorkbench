...
elseif (-not $ForceViewerRefresh -and $viewerStatus.IsCurrent) {
    Write-WbOk "Viewer artifacts are current; static regeneration skipped. Use -ForceViewerRefresh to rebuild."
}
else {
    ...
    Write-WbProgress "Generating static viewer before API startup: $viewerOutputPath"
    Write-WbInfo "Large FWD/CFD files can take a while. Future starts reuse current artifacts unless -ForceViewerRefresh is used."
