...
$diagnosticsStage = Join-Path $tempRoot 'diagnostics-package'
...
try {
    ...
    $sourceZip = New-ZipFromStage -PackageName 'source-clean' -StageRoot $sourceStage -OutputRoot $outputRoot
    $runtimeZip = New-ZipFromStage -PackageName 'runtime-local' -StageRoot $runtimeStage -OutputRoot $outputRoot
    $evidenceZip = New-ZipFromStage -PackageName 'evidence-sample' -StageRoot $evidenceStage -OutputRoot $outputRoot
    $diagnosticsZip = New-ZipFromStage -PackageName 'diagnostics-bundle' -StageRoot $diagnosticsStage -OutputRoot $outputRoot

    Write-Host ('Created split deliverables:')
    Write-Host ('  ' + $sourceZip)
    Write-Host ('  ' + $runtimeZip)
    Write-Host ('  ' + $evidenceZip)
    Write-Host ('  ' + $diagnosticsZip)
}
finally {
