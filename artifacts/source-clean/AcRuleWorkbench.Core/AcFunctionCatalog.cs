...
public static class AcFunctionCatalog
{
    ...
    private static readonly FunctionDefinition[] FunctionDefinitions =
    {
        ...
        Def("HowManyAreEmpty", "Testing", "Counts how many configured fields are empty and returns a status based on the configured threshold or branch setup.", new[] { "OK", "Failed", "None Empty", "Some Empty", "All Empty" }, new[] { "Fields to test", "Threshold or comparison option" }, new[] { "ReadsField", "BranchesRuleFlow" }, new[] { "Controls downstream Action Lists based on field completeness without changing field data." }),
        Def("lookup_DocType", "Custom / Lookup", "Custom document-type lookup function observed in this FWD. Treat exact lookup behavior as environment-specific.", new[] { "OK", "Failed", "Found", "NotFound" }, new[] { "Document type input", "Lookup table or resource", "Output fields/attributes" }, new[] { "UsesTable", "ReadsField", "MayWriteField", "BranchesRuleFlow" }, new[] { "Uses customer-specific lookup configuration; inspect Fields / Parameters and Attributes before assuming branch behavior." }, evidence: "Environment-specific observed function with curated companion metadata."),
        Def("MapCheckboxToString", "Formatting", "Maps checkbox/OMR-style field state into a configured string value.", new[] { "OK", "Failed" }, new[] { "Checkbox field", "Output field", "Mapping options" }, new[] { "ReadsField", "WritesField", "SupportsOmr" }, new[] { "Converts check state into normalized text for later validation, output, or lookup rules." }),
        Def("MapStringToCheckbox", "Formatting", "Maps a configured string value into checkbox/OMR-style field state.", new[] { "OK", "Failed" }, new[] { "Input field", "Checkbox field", "Mapping options" }, new[] { "ReadsField", "WritesField", "SupportsOmr" }, new[] { "Normalizes text values into check state used by downstream FormWorks processing." }),
        Def("ParseField33", "Formatting", "Parses provider/field 33 style text into configured component fields.", new[] { "OK", "Failed" }, new[] { "Source field", "Destination fields", "Parsing options" }, new[] { "ReadsField", "WritesField", "SupportsMultiline" }, new[] { "Splits one captured text region into structured fields used by later lookup/output rules." }),
        Def("ParseName", "Formatting", "Parses a name string into configured name component fields.", new[] { "OK", "Failed" }, new[] { "Source name field", "Destination name fields", "Parsing options" }, new[] { "ReadsField", "WritesField" }, new[] { "Normalizes captured names into structured components for matching, display, or output." }),
        Def("RectifyRangeToConstant", "Rectifying", "Rectifies character or field confidence within a configured range to a constant value.", new[] { "OK", "Failed" }, new[] { "Target field", "Range/configuration", "Constant value" }, new[] { "ReadsField", "WritesField", "UsesGeometry" }, new[] { "Can change captured data based on confidence/geometry-driven rectification configuration." }),
        Def("RectifySingleOCR", "Rectifying", "Rectifies a single OCR value using configured confidence/character rules.", new[] { "OK", "Failed" }, new[] { "Target field", "OCR/confidence options" }, new[] { "ReadsField", "WritesField", "UsesGeometry" }, new[] { "Can accept or alter an OCR result before operator repair is needed." }),
        Def("SortFields", "Formatting", "Sorts configured field or line values according to rule configuration.", new[] { "OK", "Failed" }, new[] { "Fields or lines", "Sort options" }, new[] { "ReadsField", "WritesField", "SupportsMultiline" }, new[] { "Reorders captured values so later validation/output sees a normalized sequence." }),
        Def("SubstChars", "Formatting", "Substitutes characters in configured fields according to rule options.", new[] { "OK", "Failed" }, new[] { "Target fields", "Search characters", "Replacement characters" }, new[] { "ReadsField", "WritesField" }, new[] { "Normalizes OCR/keyed text before validation, lookup, or output." }),
        Def("Uppercase", "Formatting", "Converts configured fields to uppercase.", new[] { "OK", "Failed" }, new[] { "Target fields" }, new[] { "ReadsField", "WritesField" }, new[] { "Normalizes case for matching, validation, and output consistency." })
    };
