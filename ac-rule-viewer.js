...
const storeKey='ac-rule-workbench-v43-ux-authority-workflow';
...
function readState(){
  ...
  return {
    ...
    focusNodeId:'',theme:saved.theme||'light',density:saved.density==='high'?'high':'standard',modal:'',
    ...
    recentScopes:Array.isArray(saved.recentScopes)?saved.recentScopes:[],searchActiveIndex:-1,inspectorOpen:saved.inspectorOpen===true,reviewMode:saved.reviewMode||'understand',disclosureLevel:Number(saved.disclosureLevel||2)||2
  };
}
...
function buildModel(){
  ...
  const rels=list(first(relData.Relationships,relData.Edges,[])).map((r,i)=>{
    const nodeId=correlationNodeId(r,structuralByKey,structuralByGuid,structuralByNameFn);
    const row={...r,id:`rel-${i}`,scopeId:scopeIdOf(r),nodeId,kind:text(first(r.Kind,r.EdgeKind,'Reference')),targetType:text(first(r.TargetType,'Unknown')),target:text(first(r.Target,'')),confidence:text(first(r.Confidence,'Medium')),searchBlob:''};
    row.searchBlob=[row.kind,row.targetType,row.target,row.confidence,row.ScopePath,row.RuleName,row.FunctionName].join(' ').toLowerCase();
    const scope=upsertScope(row);scope.refs++;
    return row;
  });

  const diags=list(first(treeData.Diagnostics,rulesData.Diagnostics,relData.Diagnostics,[])).map((d,i)=>{
    const severity=text(first(d.Severity,d.severity,'Info'));
    const row={...d,id:`diag-${i}`,scopeId:scopeIdOf(d),severity,title:text(first(d.Title,d.Code,d.Message,'Diagnostic')),detail:text(first(d.Detail,d.Message,d.Recommendation,'')),nodeId:text(first(d.NodeId,d.nodeId,'')),searchBlob:''};
  ...
  const addSyntheticDiag=(scopeId,severity,title,detail,nodeId='')=>{
    const row={id:`diag-synthetic-${diags.length}`,scopeId,severity,title,detail,nodeId,searchBlob:[severity,title,detail,scopeId,nodeId].join(' ').toLowerCase(),synthetic:true};
    diags.push(row);
    const scope=upsertScope({ScopePath:scopeId,scopeId});
    scope.diags++;
    if(/warn|error/i.test(severity))scope.warnings++;
  };

  incomingEdgesByChild.forEach((incoming,nodeId)=>{
    if(incoming.length>1){
      const node=nodesById.get(String(nodeId));
      addSyntheticDiag(node?.scopeId||incoming[0]?.scopeId||'Unscoped','Warning','Ambiguous incoming structural route',`${incoming.length} parent/action edges target node ${nodeId}. The UI will show the first edge for navigation, but branch authority should be reviewed.`,String(nodeId));
    }
  });

  const flowByScope=new Map();
  flowEdges.forEach(e=>{
    const id=e.scopeId||'Unscoped';
    const stats=flowByScope.get(id)||{total:0,proven:0,parsed:0,heuristic:0,unknown:0};
    stats.total++;
    const c=lower(first(e.Confidence,e.confidence,e.EdgeConfidence,''));
    if(c.includes('proven'))stats.proven++;
    else if(c.includes('parsed'))stats.parsed++;
    else if(c.includes('heur'))stats.heuristic++;
    else stats.unknown++;
    flowByScope.set(id,stats);
  });
  ...
  scopes.forEach(scope=>{
    ...
    scope.flow=flowByScope.get(scope.scopeId)||{total:0,proven:0,parsed:0,heuristic:0,unknown:0};
    ...
    if(severeCoverage){
      addSyntheticDiag(scope.scopeId,'Error','Structural coverage failure',`Flat inventory has ${fmt(scope.inventory)} row(s), but structural tree has ${fmt(scope.structural)} rule node(s). Treat this scope as audit-only until extraction/reconciliation is verified.`);
    } else if(warningCoverage){
      addSyntheticDiag(scope.scopeId,'Warning','Structural coverage warning',`Flat inventory has ${fmt(scope.inventory)} row(s) and structural tree has ${fmt(scope.structural)} rule node(s). Review flat-only inventory before sign-off.`);
    }
    if(scope.flow.total>0&&scope.flow.proven===0){
      addSyntheticDiag(scope.scopeId,'Info','Flow projection is not runtime proof',`This scope has ${fmt(scope.flow.total)} projected flow edge(s), but 0 proven runtime edge(s). Use Flow Projection only for triage.`);
    }
  });
  ...
  return {scopes:scopeList,nodes,nodesById,edges,childrenByParent,parentByChild,incomingByChild,incomingEdgesByChild,edgesByParent,rootsByScope,inventory,rels,diags,flowEdges,flowByScope,fwd:fwdData};
}
let model;
...
function scopeHealthLabel(s){return s?.health==='coverage-failure'?'Coverage failure':s?.health==='coverage-warning'?'Coverage warning':s?.health==='warning'?'Warnings':'Healthy';}
function scopeHealthNoticeHtml(s=currentScope()){
  if(!s)return '';
  const cls=scopeHealthClass(s);
  const stats=scopedRouteStats();
  const flow=s.flow||{total:0,proven:0};
  const message=s.health==='coverage-failure'
    ? `Flat inventory has ${fmt(s.inventory)} row(s), but structural tree has ${fmt(s.structural)} rule node(s). Treat this scope as audit-only until reconciled.`
    : s.health==='coverage-warning'
      ? `Flat inventory exceeds structural coverage. Review flat-only rows before sign-off.`
      : `No severe structural coverage issues detected for this scope.`;
  return `<div class="scope-health-banner ${cls}"><div><b>${esc(scopeHealthLabel(s))}</b><span>${esc(message)}</span></div><div class="health-metrics"><span>${fmt(s.structural)} structural</span><span>${fmt(s.flatOnly)} flat-only</span><span>${fmt(stats.indexOnly+stats.unresolved)} unresolved routes</span><span>${fmt(flow.proven||0)} proven flow</span></div></div>`;
}
function flowProjectionNoticeHtml(){const s=currentScope();const flow=s?.flow||{total:0,proven:0,heuristic:0,unknown:0,parsed:0};return `<div class="notice warn"><div class="notice-icon">≋</div><div><b>Flow Projection is audit-only.</b> This scope has ${fmt(flow.total)} projected edge(s), ${fmt(flow.proven)} proven runtime edge(s), ${fmt(flow.heuristic)} heuristic edge(s), and ${fmt(flow.unknown)} unknown edge(s). Do not use this as execution order.</div></div>`;}
function reviewModeButtonsHtml(){const modes=[['understand','Understand','Read a rule path'],['audit','Audit','Find risk'],['impact','Impact','Fields/tables/UDFs'],['cleanup','Cleanup','Disabled/duplicate/noise'],['explain','Explain','Copy summaries']];return `<div class="review-modes" role="group" aria-label="Review modes">${modes.map(([id,label,title])=>`<button class="review-mode ${state.reviewMode===id?'active':''}" type="button" data-review-mode="${id}" title="${esc(title)}" aria-pressed="${state.reviewMode===id?'true':'false'}">${esc(label)}</button>`).join('')}</div>`;}
function reviewModeIntroHtml(){const copy={understand:'Read a structural rule path and inspect actions.',audit:'Prioritize diagnostics, coverage failures, and unresolved evidence.',impact:'Focus on fields, tables, attributes, sources, and UDF dependencies.',cleanup:'Find disabled branches, flat-only rows, ambiguous matches, and high-noise areas.',explain:'Generate a business-readable explanation with evidence caveats.'};return `<div class="mode-intro"><b>${esc(state.reviewMode[0].toUpperCase()+state.reviewMode.slice(1))} mode</b><span>${esc(copy[state.reviewMode]||copy.understand)}</span></div>`;}
function dependencySummaryCardsHtml(){const fieldIndex=getScopeFieldResolutionIndex(state.scopeId);const refs=scopedRels();const tables=refs.filter(r=>/table|lookup|selection/i.test(`${r.targetType} ${r.kind} ${r.target}`)).length;const attrs=refs.filter(r=>/attr|attribute/i.test(`${r.targetType} ${r.kind}`)).length;const sources=refs.filter(r=>/source|fileref|file/i.test(`${r.targetType} ${r.kind} ${r.target}`)).length;return `<div class="metric-grid"><div class="metric ${fieldIndex.summary.unresolved?'warn':'good'}"><b>${fmt(fieldIndex.summary.unresolved)}</b><span>Unresolved fields</span></div><div class="metric"><b>${fmt(tables)}</b><span>Table/lookup refs</span></div><div class="metric"><b>${fmt(attrs)}</b><span>Attribute refs</span></div><div class="metric"><b>${fmt(sources)}</b><span>Source/file refs</span></div></div>`;}
...
function renderMainHead(){
  ...
  const captions={
    ...
    dependencies:'Fields, tables, attributes, sources, drivers, and UDF impact evidence',
    ...
    audit:'Diagnostics, provenance, route coverage, and extraction audit'
  };
...
function renderContent(){
  if(state.workspaceView==='dependencies')return renderDependenciesWorkspace();
...
function renderDependenciesWorkspace(){
  const s=currentScope();
  const refs=scopedRels();
  const fieldIndex=getScopeFieldResolutionIndex(s.scopeId);
  const highRefs=refs.filter(r=>lower(r.confidence)==='high').length;
  const fieldRows=fieldIndex.rows.slice(0,80).map(r=>`<div class="data-row compact" data-node="${esc(r.nodeId)}"><div><div class="data-title">${esc(r.referencedField)}</div><div class="data-sub">${esc(r.ruleName)} · ${esc(r.parameterName)} = ${esc(r.parameterValue)}</div></div><span class="badge ${r.fieldExists?'green':'amber'}">${r.fieldExists?'resolved':'unresolved'}</span><span class="mono">${esc(r.functionName||'')}</span></div>`).join('')||'<div class="muted">No field-like parameters detected for this scope.</div>';
  const refRows=refs.slice(0,120).map(r=>`<div class="data-row compact" data-rel="${esc(r.id)}"><div><div class="data-title">${esc(r.kind)} → ${esc(r.target||'(empty)')}</div><div class="data-sub">${esc(r.RuleName||r.SourceRuleName||r.scopeId)} · ${esc(r.fn||r.FunctionName||'')}</div></div>${badgeConfidence(r.confidence)}<span class="mono">${esc(r.targetType)}</span></div>`).join('')||'<div class="muted">No relationship evidence for this scope.</div>';
  const lenses=`<div class="dependency-lenses"><button class="quick-card" type="button" data-action="view-field-resolution"><b>Fields</b><span>${fmt(fieldIndex.summary.resolved)} resolved / ${fmt(fieldIndex.summary.unresolved)} unresolved</span></button><button class="quick-card" type="button" data-action="view-tables"><b>Tables</b><span>Canonical definitions and inferred usage</span></button><button class="quick-card" type="button" data-action="view-udfs"><b>UDFs</b><span>Function candidates and caller evidence</span></button><button class="quick-card" type="button" data-action="view-resources"><b>Resources</b><span>Sources, attributes, drivers, and resource buckets</span></button></div>`;
  $('content').innerHTML=`<section class="dependency-workspace">${scopeHealthNoticeHtml()}<div class="notice"><div class="notice-icon">⇄</div><div><b>Dependencies are static evidence.</b> These lists show field, table, attribute, source, and UDF references inferred or matched from the extraction. They do not prove runtime branch execution.</div></div>${dependencySummaryCardsHtml()}<div class="panel"><h3>Dependency lenses</h3>${lenses}</div><div class="grid-2"><div class="panel"><h3>Field impact matrix</h3>${fieldRows}</div><div class="panel"><h3>Relationship evidence</h3><div class="caption mb-8">${fmt(highRefs)} high-confidence reference(s), ${fmt(refs.length)} total reference row(s).</div>${refRows}</div></div></section>`;
}
function reviewQueues(){
  const s=currentScope();
  const nodes=scopedRuleNodes();
  const fieldIndex=getScopeFieldResolutionIndex(s.scopeId);
  const queues=[
    {id:'coverage',label:'Structural coverage failures',count:s.health==='coverage-failure'?1:0,query:'Structural coverage failure',view:'audit',severity:s.health==='coverage-failure'?'bad':'good'},
    {id:'flat',label:'Flat-only inventory rows',count:s.flatOnly||0,query:'flatonly:true',view:'audit',severity:s.flatOnly?'warn':'good'},
    {id:'routes',label:'Unresolved branch labels',count:scopedRouteStats().indexOnly+scopedRouteStats().unresolved,query:'route:Unresolved',view:'structure',severity:(scopedRouteStats().indexOnly+scopedRouteStats().unresolved)?'warn':'good'},
    {id:'disabled',label:'Disabled / inherited-disabled rules',count:nodes.filter(n=>n.disabled!=='none').length,query:'has:disabled',view:'structure',severity:'info'},
    {id:'diag',label:'Diagnostics linked to scope',count:scopedDiags().filter(d=>/warn|error/i.test(d.severity)).length,query:'diagnostic',view:'audit',severity:scopedDiags().some(d=>/warn|error/i.test(d.severity))?'warn':'good'},
    {id:'fields',label:'Unresolved field references',count:fieldIndex.summary.unresolved,query:'',view:'dependencies',severity:fieldIndex.summary.unresolved?'warn':'good'}
  ];
  return queues;
}
function renderReviewQueuesHtml(){return `<div class="review-queue-grid">${reviewQueues().map(q=>`<button class="review-queue ${q.severity}" type="button" data-review-queue="${esc(q.id)}"><b>${esc(q.label)}</b><span>${fmt(q.count)} item${q.count===1?'':'s'}</span></button>`).join('')}</div>`;}
function renderAuditWorkspace(){
  const s=currentScope(),stats=scopedRouteStats(),diags=scopedDiags(),refs=scopedRels(),inv=scopedInventory(),flow=s.flow||{total:0,proven:0,parsed:0,heuristic:0,unknown:0};
  const diagRows=diags.length?diags.slice(0,250).map(d=>`<div class="data-row compact ${state.selectedId===d.id?'selected':''}" data-diag="${esc(d.id)}"><div><div class="data-title">${esc(d.title)}</div><div class="data-sub">${esc(d.detail||d.Message||'')}</div></div><span class="badge ${/warn|error/i.test(d.severity)?'amber':'blue'}">${esc(d.severity)}</span><span class="mono">${esc(d.scopeId||'Unscoped')}</span></div>`).join(''):'<div class="muted">No diagnostics for this scope.</div>';
  const flatOnly=inv.filter(r=>r.classification==='FlatOnly');
  const flatRows=flatOnly.length?flatOnly.slice(0,120).map(r=>`<div class="data-row compact" data-inventory="${esc(r.id)}"><div><div class="data-title">${esc(r.title)}</div><div class="data-sub">${esc(r.fn||'no function')} · ${esc(r.RuleGuid||r.RuleId||'no id')}</div></div><span class="badge amber">flat-only</span></div>`).join(''):'<div class="muted">No flat-only inventory rows in this scope.</div>';
  $('content').innerHTML=`<section class="tables-workbench audit-workspace">${scopeHealthNoticeHtml()}${flowProjectionNoticeHtml()}<div class="panel"><h3>Review queues</h3><p class="caption">Start here. Queues are task-based so reviewers do not need to invent fragile searches.</p>${renderReviewQueuesHtml()}</div><div class="metric-grid"><div class="metric good"><b>${fmt(scopedRuleNodes().length)}</b><span>Structural rules</span></div><div class="metric ${stats.indexOnly+stats.unresolved?'warn':'good'}"><b>${fmt(stats.indexOnly+stats.unresolved)}</b><span>Unresolved routes</span></div><div class="metric ${diags.some(d=>/warn|error/i.test(d.severity))?'warn':'good'}"><b>${fmt(diags.length)}</b><span>Diagnostics</span></div><div class="metric warn"><b>${fmt(flow.total)}</b><span>Flow projection edges</span></div></div><div class="grid-2"><div class="panel"><h3>Scope audit summary</h3><div class="kv">${kv('Scope ID',esc(s.scopeId))}${kv('Health',`<span class="badge ${scopeHealthClass(s)==='bad'?'red':scopeHealthClass(s)==='warn'?'amber':'green'}">${esc(scopeHealthLabel(s))}</span>`)}${kv('Inventory rows',fmt(inv.length))}${kv('Flat-only rows',fmt(flatOnly.length))}${kv('References',fmt(refs.length))}${kv('Snapshot',esc(snapshotId()))}</div></div><div class="panel"><h3>Flow projection summary</h3><div class="kv">${kv('Total projected edges',fmt(flow.total))}${kv('Proven runtime edges',fmt(flow.proven||0))}${kv('Parsed edges',fmt(flow.parsed||0))}${kv('Heuristic edges',fmt(flow.heuristic||0))}${kv('Unknown edges',fmt(flow.unknown||0))}</div></div></div><div class="panel"><h3>Diagnostics</h3>${diagRows}</div><div class="panel"><h3>Flat-only inventory</h3>${flatRows}</div></section>`;
}
...
function usageRowsForDefinition(matches){
  ...
  list(model.rels).forEach(r=>{
    ...
    seen.add(key);
    rows.push({rel:r,node,scopeId:text(r.scopeId||node?.scopeId||'Unscoped'),ruleName:text(r.RuleName||r.SourceRuleName||node?.title||r.target||'Reference'),functionName:text(r.fn||r.FunctionName||node?.fn||''),target:text(r.target||''),targetType:text(r.targetType||''),relationshipKind:text(r.kind||''),confidence:text(r.confidence||'')});
  });
  ...
  if(buckets.length){
    ...
    return buckets.flatMap(bucket=>{
      ...
      return list(bucket.names).map(item=>{
        ...
        return {key:`${type}|${name}`,name,type,source:'Canonical FWD resource',canonical:true,metric:Number(first(item?.usedByRuleCount,item?.count,usage.length,0))||usage.length,usage,details:first(item?.details,item?.Details,null)};
      }).filter(r=>r.name);
    }).sort((a,b)=>a.type.localeCompare(b.type,undefined,{sensitivity:'base'})||a.name.localeCompare(b.name,undefined,{sensitivity:'base'}));
  }
  return domainRowsByView('resources').map(r=>({key:text(r.name),name:text(r.name),type:'Resource evidence',source:'Relationship evidence',canonical:false,metric:Number(first(r.count,0))||0,usage:[],details:null}));
}
function buildGlobalDriverDefinitions(){
  ...
  if(items.length){
    return items.map(item=>{
      ...
      return {key:name,name,type:text(item.classification||'Process driver'),source:text(item.source||'Canonical process config'),canonical:true,metric:Number(first(item.findingCount,findings.length,usage.length,0))||0,usage:[...findings,...usage],details:item};
    }).sort((a,b)=>a.name.localeCompare(b.name,undefined,{sensitivity:'base'}));
  }
  return domainRowsByView('drivers').map(r=>({key:text(r.name),name:text(r.name),type:'Driver evidence',source:'Relationship evidence',canonical:false,metric:Number(first(r.count,0))||0,usage:[],details:null}));
}
...
function definitionListHtml(kind,rows,selectedKey){
  return `<div class="global-list-head"><span>Definitions</span><b>${fmt(rows.length)}</b></div><div class="table-index-list global-def-list">${rows.slice(0,800).map(row=>`<button class="table-index-row ${row.key===selectedKey?'active':''}" type="button" data-global-kind="${esc(kind)}" data-global-key="${esc(row.key)}"><span class="table-index-main"><b>${esc(row.name)}</b><span>${esc(row.type)} · ${fmt(list(row.usage).length||row.metric)} evidence</span></span><span class="table-index-side"><span class="badge ${row.canonical?'green':'amber'}">${row.canonical?'Canonical':'Inferred'}</span></span></button>`).join('')}</div>`;
}
function usagePreviewHtml(rows){
  const usage=list(rows);
  if(!usage.length)return '<div class="global-empty-state">No mapped usage evidence for this definition in the current snapshot.</div>';
  return `<div class="global-usage-preview">${usage.slice(0,8).map(row=>`<div class="global-usage-mini"><div><b>${esc(row.ruleName)}</b><span>${esc(row.scopeId)} · ${esc(row.functionName||row.relationshipKind||row.targetType||'Evidence')}</span></div>${row.node?`<button class="btn ghost" type="button" data-node="${esc(row.node.id)}">Open</button>`:'<span class="badge amber">Evidence</span>'}</div>`).join('')}</div>${usage.length>8?`<div class="caption mt-8">${fmt(usage.length-8)} more row(s) in Explore evidence.</div>`:''}`;
}
function parameterMatrixHtml(callers){
  const byName=new Map();
  list(callers).forEach(c=>Object.entries(c.parameters||{}).forEach(([name,raw])=>{
    const key=text(name).trim();if(!key)return;
    if(!byName.has(key))byName.set(key,{name:key,values:new Map(),callers:new Set()});
    const row=byName.get(key);row.callers.add(text(c.nodeId||c.ruleName));
    list(raw).forEach(v=>{const value=text(v);row.values.set(value,(row.values.get(value)||0)+1);});
  }));
  ...
  return `<div class="global-param-matrix">${rows.map(row=>`<div class="global-param-card"><b>${esc(row.name)}</b><span>${fmt(row.callers.size)} caller rule(s)</span><div>${[...row.values.entries()].sort((a,b)=>b[1]-a[1]).slice(0,4).map(([v,c])=>`<span class="badge blue" title="${esc(v)}">${esc(v||'(blank)')} ${fmt(c)}</span>`).join('')}</div></div>`).join('')}</div>`;
}
function globalDetailRecord(){
  if(state.globalDetailKind==='resources')return {kind:'resources',label:'Resource evidence',row:buildGlobalResourceDefinitions().find(r=>r.key===state.selectedResourceKey)};
  if(state.globalDetailKind==='drivers')return {kind:'drivers',label:'Driver evidence',row:buildGlobalDriverDefinitions().find(r=>r.key===state.selectedDriverKey)};
  if(state.globalDetailKind==='tables'){
    const table=buildGlobalTableDefinitions().find(r=>r.name===state.selectedTableName);
    return table?{kind:'tables',label:'Table evidence',row:{...table,key:table.name,name:table.name,type:table.hasParsedSchema?'Parsed table':'Table',source:table.inferred?'Evidence-derived':'Canonical payload',usage:list(table.usage)}}:null;
  }
  if(state.globalDetailKind==='udfs'){
    const udf=buildUdfDefinitions().find(r=>r.key===state.selectedUdfName);
    return udf?{kind:'udfs',label:'UDF caller evidence',row:{...udf,name:udf.displayName||udf.key}}:null;
  }
  return null;
}
function renderGlobalEvidenceModal(){
  ...
  if(record.kind==='udfs'){
    ...
    return `<div class="global-modal-shell"><div class="global-modal-summary">${summaryTilesHtml([{label:'Parameters',value:fmt(list(row.parameterNames).length)},{label:'Caller rules',value:fmt(callers.length)},{label:'Type',value:esc(row.type)}])}</div><div class="table-columns-head">Parameters</div>${parameterMatrixHtml(callers)}<div class="table-columns-head">Caller hierarchy</div>${usagePreviewHtml(callers.map(c=>({scopeId:c.scopeId,ruleName:c.ruleName,functionName:c.functionName,node:c.nodeId?model.nodesById.get(String(c.nodeId)):null,target:'',targetType:'UDF caller',relationshipKind:'Calls'})))}</div>`;
  }
  const usage=list(row.usage);
  return `<div class="global-modal-shell"><div class="global-modal-summary">${summaryTilesHtml([{label:'Evidence rows',value:fmt(usage.length)},{label:'Source',value:esc(row.source||'Evidence')},{label:'Type',value:esc(row.type||'Definition')}])}</div><div class="table-columns-head">Usage evidence</div>${usage.length?`<div class="global-evidence-list">${usage.slice(0,160).map(u=>`<div class="global-evidence-row"><div><b>${esc(u.ruleName)}</b><span>${esc(u.scopeId)} · ${esc(u.functionName||u.relationshipKind||u.targetType||'Evidence')}</span></div>${u.node?`<button class="btn ghost" type="button" data-node="${esc(u.node.id)}">Open in tree</button>`:`<span class="badge amber">Evidence only</span>`}<div class="definition-preview">${esc(u.target||'')}</div></div>`).join('')}</div>`:'<div class="global-empty-state">No usage evidence is mapped for this definition.</div>'}</div>`;
}
function renderGlobalDefinitionExplorer(kind,rows,selectedKey,stateKey,copy,detailHtml){
  if(!rows.length){$('content').innerHTML=`<section class="global-explorer">${globalHeroHtml(copy,[{label:'Definitions',value:0}])}${emptyHtml(copy.emptyTitle,copy.emptyBody)}</section>`;return;}
  const selected=rows.find(r=>r.key===selectedKey)||rows[0];
  state[stateKey]=selected.key;
  const canonical=rows.filter(r=>r.canonical).length;
  const withUsage=rows.filter(r=>list(r.usage).length>0).length;
  $('content').innerHTML=`<section class="global-explorer">${globalHeroHtml(copy,[{label:'Definitions',value:rows.length},{label:'Canonical',value:canonical},{label:'With evidence',value:withUsage}])}<div class="global-explorer-grid"><aside class="panel global-index-panel">${definitionListHtml(kind,rows,selected.key)}</aside><section class="panel global-detail-panel"><div class="global-detail-head"><div><h3>${esc(selected.name)}</h3><p>${esc(selected.type)} · ${esc(selected.source)}</p></div><div class="tree-detail-badges"><span class="badge ${selected.canonical?'green':'amber'}">${selected.canonical?'Canonical':'Inferred'}</span><span class="badge blue">${fmt(list(selected.usage).length||selected.metric)} evidence</span></div></div>${summaryTilesHtml([{label:'Type',value:esc(selected.type)},{label:'Source',value:esc(selected.source)},{label:'Usage',value:fmt(list(selected.usage).length)}])}${detailHtml(selected)}<div class="global-detail-actions"><button class="btn primary" type="button" data-action="open-global-detail" data-global-kind="${esc(kind)}">Explore evidence</button></div></section></div></section>`;
}
function renderGlobalResourceDefinitions(){
  renderGlobalDefinitionExplorer('resources',buildGlobalResourceDefinitions(),state.selectedResourceKey,'selectedResourceKey',{title:'Resources',body:'Global resources are shared definitions. The main panel stays focused on identity and usage status; dense evidence opens separately.',emptyTitle:'No resources found',emptyBody:'No canonical resources or relationship-derived resources were discovered.'},row=>`<div class="table-columns-head">Usage preview</div>${usagePreviewHtml(row.usage)}`);
}
function renderGlobalDriverDefinitions(){
  renderGlobalDefinitionExplorer('drivers',buildGlobalDriverDefinitions(),state.selectedDriverKey,'selectedDriverKey',{title:'Input / Output Drivers',body:'Driver definitions and process-private findings are global configuration evidence, not page or document children.',emptyTitle:'No drivers found',emptyBody:'No process driver definitions or driver-like evidence were discovered.'},row=>`<div class="table-columns-head">Finding preview</div>${usagePreviewHtml(row.usage)}`);
}
...
function buildGlobalTableDefinitions(){
  ...
  const usageByTarget=tableUsageIndex(canonicalTables.map(t=>t.name));
...
function renderGlobalTablesMasterDetail(){
  const tables=buildGlobalTableDefinitions();
  const isCanonical=list(model.fwd?.tables?.items).length>0;
  const explorerRows=tables.map(t=>({key:t.name,name:t.name,type:t.hasParsedSchema?'Parsed table':'Table',source:t.inferred?'Evidence-derived':'Canonical payload',canonical:t.canonical,metric:t.hits,usage:list(t.usage),table:t}));
  if(!tables.length){
    $('content').innerHTML=`<section class="global-explorer">${globalHeroHtml({title:'Tables',body:'No table definitions were discovered in this snapshot.'},[{label:'Definitions',value:0}])}${emptyHtml('No tables found','No table definitions were discovered in canonical resources or relationship evidence.')}</section>`;
    return;
  }
  ...
  renderGlobalDefinitionExplorer('tables',explorerRows,state.selectedTableName,'selectedTableName',{title:'Tables',body:isCanonical?'Canonical table definitions with parsed schema and usage-derived fields separated from caller evidence.':'Table candidates are inferred from static relationship evidence.',emptyTitle:'No tables found',emptyBody:'No table definitions were discovered.'},row=>{
    const t=row.table;
    return `<div class="table-columns-head">Fields / columns</div>${list(t.usageDerivedFields).length?`<div class="table-columns-grid">${list(t.usageDerivedFields).map(c=>`<div class="table-column-row"><div class="table-col-name">${esc(c.name)}</div><div class="table-col-meta"><span class="badge blue">${fmt(c.hits)} uses</span><span class="mono">${esc(c.confidence)} confidence</span></div></div>`).join('')}</div>`:'<div class="global-empty-state">No parsed schema or usage-derived fields extracted.</div>'}<div class="table-columns-head">Usage preview</div>${usagePreviewHtml(row.usage)}`;
  });
  return;
  const canonicalCount=tables.filter(t=>t.canonical).length;
  const inferredCount=Math.max(0,tables.length-canonicalCount);
...
function renderUdfMasterDetail(){
  ...
  const udfExplorerRows=rows.map(r=>({key:r.key,name:r.displayName,type:r.type,source:r.source||'Candidate evidence',canonical:r.canonical,metric:list(r.callerRules).length,usage:list(r.callerRules).map(c=>({scopeId:c.scopeId,ruleName:c.ruleName,functionName:c.functionName,node:c.nodeId?model.nodesById.get(String(c.nodeId)):null,target:'',targetType:'UDF caller',relationshipKind:'Calls'})),udf:r}));
  renderGlobalDefinitionExplorer('udfs',udfExplorerRows,state.selectedUdfName,'selectedUdfName',{title:'User Defined Functions',body:'UDFs are global functions. Parameters and caller hierarchy are summarized here; heavy caller evidence opens in a focused view.',emptyTitle:'No UDFs found',emptyBody:'No UDF candidates match the current filter.'},row=>{
    const u=row.udf;
    const callers=list(u.callerRules);
    return `<div class="segmented mb-10">${['with-callers','canonical','unparsed','relationship-only','all'].map(f=>`<button class="${state.udfFilter===f?'active':''}" type="button" data-udf-filter="${f}">${esc(udfFilterLabel(f))}</button>`).join('')}</div><div class="table-columns-head">Parameters</div>${parameterMatrixHtml(callers)}<div class="table-columns-head">Caller preview</div>${usagePreviewHtml(row.usage)}`;
  });
  return;
...
function rulePlainLanguageNarrative(n){
  ...
  return [`Rule: ${n.title}`,`Function: ${n.fn||'No mapped function'}`,`Scope: ${n.scopeId}`,`State: ${disabled}`,incoming?`Runs under parent action: ${incoming.label}${incoming.resolved?'':' (label unresolved/index-only)'}`:'Root rule-list entry',fields.length?`Field-like inputs: ${fields.join(', ')}`:'No field-like inputs detected',branches.length?`Outgoing actions: ${branches.map(b=>`${b.label} (${b.childIds.length} child rules)`).join('; ')}`:'No child action branches detected','Evidence caveat: this is static structural/extraction evidence, not a runtime execution trace.'].join('\n');
}
function pathNarrativeHtml(n){
  const lines=rulePlainLanguageNarrative(n).split('\n');
  return `<div class="path-narrative"><h3>Path narrative</h3><ul>${lines.map(line=>`<li>${esc(line)}</li>`).join('')}</ul><div class="branch-actions"><button class="btn" type="button" data-action="copy-rule-explanation">Copy explanation</button><button class="btn" type="button" data-action="copy-route-path">Copy route JSON</button></div></div>`;
}
...
function requestedWorkspaceView(){
  try{
    ...
    return ['structure','dependencies','field-resolution','resources','tables','drivers','udfs','audit'].includes(view)?view:'';
  }catch{return '';}
}
...
function saveState(){try{localStorage.setItem(snapshotStoreKey(),JSON.stringify({scopeId:state.scopeId,theme:state.theme,density:state.density,treeFilter:state.treeFilter,scopeKindFilter:state.scopeKindFilter,workspaceView:state.workspaceView,fieldResolutionFilter:state.fieldResolutionFilter,selectedResourceKey:state.selectedResourceKey,selectedDriverKey:state.selectedDriverKey,selectedTableName:state.selectedTableName,selectedUdfName:state.selectedUdfName,udfFilter:state.udfFilter,inspectorOpen:document.body.classList.contains('inspector-open'),recentScopes:state.recentScopes,reviewMode:state.reviewMode,disclosureLevel:state.disclosureLevel}));localStorage.setItem('ac-rule-workbench-theme',state.theme);}catch{}}
function restoreSnapshotState(){const saved=safeJson(localStorage.getItem(snapshotStoreKey())||'{}',{});const theme=localStorage.getItem('ac-rule-workbench-theme')||saved.theme||state.theme||'light';state.theme=theme;document.documentElement.dataset.theme=theme;state.density=saved.density==='high'?'high':state.density;applyDensityClass(state.density);if(saved.scopeId&&model.scopes.some(s=>s.scopeId===saved.scopeId))state.scopeId=saved.scopeId;if(saved.treeFilter)state.treeFilter=saved.treeFilter;if(saved.scopeKindFilter)state.scopeKindFilter=saved.scopeKindFilter;state.workspaceView=['structure','dependencies','field-resolution','resources','tables','drivers','udfs','audit'].includes(saved.workspaceView)?saved.workspaceView:'structure';state.workspaceView=requestedWorkspaceView()||state.workspaceView;state.fieldResolutionFilter=['all','resolved','unresolved'].includes(saved.fieldResolutionFilter)?saved.fieldResolutionFilter:'unresolved';state.selectedResourceKey=text(saved.selectedResourceKey||'');state.selectedDriverKey=text(saved.selectedDriverKey||'');state.selectedTableName=text(saved.selectedTableName||'');state.selectedUdfName=text(saved.selectedUdfName||'');state.udfFilter=['all','with-callers','canonical','unparsed','relationship-only'].includes(saved.udfFilter)?saved.udfFilter:state.udfFilter;state.recentScopes=Array.isArray(saved.recentScopes)?saved.recentScopes:[];state.reviewMode=saved.reviewMode||state.reviewMode||'understand';state.disclosureLevel=Number(saved.disclosureLevel||state.disclosureLevel||2)||2;if(saved.inspectorOpen)document.body.classList.add('inspector-open');}
...
function selectNode(id){if(state.modal)state.modal='';state.workspaceView='structure';state.selectedType='node';state.selectedId=String(id);state.expanded.add(String(id));let child=String(id);let p=model.parentByChild.get(child);while(p){state.expanded.add(p);const incoming=model.incomingByChild.get(child);if(incoming)state.collapsedBranches.delete(branchKeyFromEdge(p,incoming));child=p;p=model.parentByChild.get(p);}document.body.classList.add('inspector-open');renderAll();setTimeout(()=>{const row=document.querySelector(`[data-node="${cssEscape(String(id))}"]`);row?.scrollIntoView({block:'nearest'});row?.focus();},0);}
function branchRow(r){
  ...
  const actionLabel=g.resolved?'Resolved action':'Unresolved/index-only action';
  return `<div class="branch-row ${cls} ${open?'':'collapsed'} ${selected?'selected':''} ${hot?'hotspot':''}" role="treeitem" aria-level="${r.level+1}" aria-expanded="${open?'true':'false'}" aria-selected="${selected?'true':'false'}" tabindex="0" data-branch="${esc(key)}" style="--depth:${r.level}"><button class="twisty branch-twisty" type="button" data-toggle-branch="${esc(key)}" aria-label="${open?'Collapse':'Expand'} action ${esc(g.label)}">${open?'−':'+'}</button><div class="branch-main"><span class="branch-label"><span class="route-prefix">${esc(actionLabel)}</span> ${esc(g.label)}</span><span class="branch-meta">${fmt(g.childIds.length)} child ${g.childIds.length===1?'rule':'rules'} · ${esc(g.routeState||'Route')}</span></div><button class="mini-row-btn" type="button" data-toggle-branch="${esc(key)}" aria-label="${open?'Collapse':'Expand'} branch">${open?'−':'+'}</button></div>`;
}
...
function renderScopes(){
  ...
  function scopeRowHtml(s){
    ...
    const flatText=s.flatOnly?`${fmt(s.flatOnly)} flat-only`:'no flat-only';
    return `<button class="scope-row ${active?'active':''} health-${cls}" type="button" data-scope="${esc(s.scopeId)}" aria-current="${active?'true':'false'}"><span class="scope-icon" aria-hidden="true">${icon}</span><span class="scope-row-main"><span class="scope-name">${esc(s.name)}</span><span class="scope-row-meta">${esc(s.scopeId)}</span><span class="scope-row-evidence"><span class="mini-health ${cls}">${esc(label)}</span><span>${fmt(s.structural)} structural</span><span>${flatText}</span></span></span></button>`;
  }
...
function activeSliceHtml(){
  ...
  const labels={
    ...
    dependencies:['Dependencies','Impact evidence'],
    ...
    audit:['Audit','Coverage and risk']
  };
  const row=labels[view]||['Workbench','Evidence view'];
...
function renderViewbar(){
  ...
  const dep=state.workspaceView==='dependencies'||['field-resolution','resources','tables','drivers','udfs'].includes(state.workspaceView);
  const audit=state.workspaceView==='audit';
  const trustLine='Structure first · dependencies second · audit evidence always available · no runtime execution simulated';
  const viewButtons=[
    `<button class="btn ${struct?'primary':''}" type="button" data-action="view-structure" title="Inspect rule hierarchy, action branches, and disabled inheritance">Structure</button>`,
    `<button class="btn ${dep?'primary':''}" type="button" data-action="view-dependencies" title="Review fields, tables, attributes, sources, drivers, and UDF evidence">Dependencies</button>`,
    `<button class="btn ${audit?'primary':''}" type="button" data-action="view-audit" title="Review diagnostics, coverage failures, and flow projection caveats">Audit</button>`
  ];
  const treeCommands=struct
    ? `<div class="cmd-main" role="group" aria-label="Tree commands"><button class="btn primary" type="button" data-action="expand-selected-subtree" title="Expand the selected rule and all descendants" ${hasRule?'':'disabled'}>Expand selected</button><button class="btn" type="button" data-action="expand-all" title="Expand all visible structural rules and branches">Expand all</button><button class="btn" type="button" data-action="collapse-all" title="Collapse to top-level roots and branch headers">Collapse all</button>${state.focusNodeId?'<button class="btn" type="button" data-action="clear-focus" title="Return from focused subtree to full scope">Clear focus</button>':''}</div><div class="cmd-hint">Keyboard: / search · arrows move · Enter selects · Space expands · ? opens help.</div>`
    : '<div class="cmd-hint">Use Structure for hierarchy, Dependencies for impact, and Audit for confidence/risk.</div>';
  const html=`<div class="viewbar-shell"><div class="viewbar-left">${activeSliceHtml()}${reviewModeButtonsHtml()}${reviewModeIntroHtml()}<div class="trust-line">${esc(trustLine)}</div><div class="cmd-main" role="group" aria-label="Workbench views">${viewButtons.join('')}</div>${renderContextActionMenu('Scope')}<div class="field tree-filter"><label class="sr-only" for="viewSearch">Filter current view</label><input id="viewSearch" type="search" value="${esc(state.query)}" placeholder="Filter by rule, function, field, table, attribute, route, or risk"><button class="filter-clear" type="button" data-action="clear-tree-search" title="Clear current local filter" aria-label="Clear current filter" ${hasFilter?'':'disabled'}>Clear</button></div>${treeCommands}</div><div class="viewbar-right"><select id="treeFilter" ${struct?'':'disabled'} aria-label="Tree filter"><option value="all" ${state.treeFilter==='all'?'selected':''}>All structural nodes</option><option value="disabled" ${state.treeFilter==='disabled'?'selected':''}>Disabled only</option><option value="inherited" ${state.treeFilter==='inherited'?'selected':''}>Inherited disabled</option><option value="warnings" ${state.treeFilter==='warnings'?'selected':''}>Diagnostics only</option><option value="actions" ${state.treeFilter==='actions'?'selected':''}>Action parents</option><option value="sections" ${state.treeFilter==='sections'?'selected':''}>Sections/comments</option></select><select id="disclosureLevel" aria-label="Tree disclosure level"><option value="1" ${state.disclosureLevel===1?'selected':''}>Level 1: names</option><option value="2" ${state.disclosureLevel===2?'selected':''}>Level 2: function</option><option value="3" ${state.disclosureLevel===3?'selected':''}>Level 3: state</option><option value="4" ${state.disclosureLevel===4?'selected':''}>Level 4: evidence</option></select><span class="view-count">${esc(viewLabel())}</span></div></div>`;
...
function renderStructure(){
  ...
  const treeHtml=rows.length
    ?`${scopeHealthNoticeHtml()}${treeSelectionPanelHtml()}<div class="tree" role="tree" aria-label="Structural rule tree">${rows.map(r=>r.type==='branch'?branchRow(r):treeRow(r.n,r.level)).join('')}</div>`
...
function viewSearchMeta(){
  ...
  if(state.workspaceView==='dependencies')return {label:'Filter dependencies',placeholder:'Filter field, table, attribute, source, UDF, or relationship evidence'};
  ...
  if(state.workspaceView==='audit')return {label:'Filter audit',placeholder:'Filter diagnostics, references, and route coverage'};
