<#
.SYNOPSIS
  Root-friendly one-command launcher for FW Editor Viewer.
...
Write-Section "FW Editor Viewer local launcher"
...
if (-not $SkipUnblock) {
    ...
    Write-ProgressLine "Removing downloaded-file blocks from $($files.Count) local file(s)."
...
Write-Section "Hand off to startup pipeline"
...
Write-ProgressLine "The startup pipeline will build if needed, open the viewer, and print [COMPLETE] after server readiness."
