...
public sealed class ViewerExportTests
{
    ...
    public void ExportAcViewer_Writes_SidecarJsonFiles()
    {
        ...
        string outHtml = Path.Combine(repoRoot, "ac-viewer-test.html");
        ...
        string flow = Path.Combine(repoRoot, "ac-rule-viewer.flow.json");
        ...
        Assert.IsTrue(File.Exists(flow), "Missing flow sidecar: " + flow);
        ...
        Assert.IsTrue(new FileInfo(flow).Length > 10, "flow JSON appears empty");
